...
import { captureFrameFfmpeg, captureFrameFromTag } from "./capture-frame";
import { controlsBackground, primaryColor } from "./colors";
import ValueTuner from "./components/ValueTuner";
import
  {
    askForFileOpenAction, askForHtml5ifySpeed, askForImportChapters, askForOutDir,
    ...
    cleanupFilesDialog, confirmExtractAllStreamsDialog, createFixedDurationSegments, createNumSegments,
    ...
    openAbout, openYouTubeChaptersDialog, promptTimeOffset,
    ...
    showCutFailedDialog, showDiskFull,
...
import
  {
    ...
    loadMplayerEdl, loadPbf, loadXmeml, saveCsv,
    ...
    saveCsvHuman, saveTsv
  } from "./edlStore";
import ExportConfirm from "./ExportConfirm";
import
  {
    createChaptersFromSegments, defaultProcessedCodecTypes,
    ...
    extractStreams,
    getAllStreams, getDefaultOutFormat,
    ...
    getDuration, getFormatData, getStreamFps,
    ...
    getTimecodeFromStreams, isAudioSupported, isCuttingEnd, isCuttingStart,
    ...
    isIphoneHevc, isStreamThumbnail, renderThumbnails as ffmpegRenderThumbnails,
...
import
  {
    createInitialCutSegments, createSegment,



    findSegmentsAtCursor, getCleanCutSegments,
    getSegApparentStart,


    invertSegments, sortSegments
  } from "./segments";
...
import
  {
    ...
    defaultOutSegTemplate, dirExists,




    doesPlayerSupportFile, dragPreventer, errorToast,
    ...
    getOutFileExtension, getOutPath,
    ...
    hasDuplicates,
    havePermissionToReadFile, isDurationValid,
    ...
    isWindows, openDirToast, setFileNameTitle, showFfmpegFail, toast,
...
const { unlink, exists, readdir, statSync, unlinkSync, renameSync } = window.require("fs-extra");
const {
	extname,
	parse: parsePath,
	sep: pathSep,
...
const calcShouldShowWaveform = (zoomedDuration) =>
	zoomedDuration != null && zoomedDuration < ffmpegExtractWindow * 8;
const calcShouldShowKeyframes = (zoomedDuration) =>
	zoomedDuration != null && zoomedDuration < ffmpegExtractWindow * 8;
...
const App = memo(() => {
	...
	const zoomedDuration = isDurationValid(duration)
		? duration / zoom
		: undefined;
	...
	const {
		mergeFiles: ffmpegMergeFiles,
		html5ifyDummy,
		cutMultiple,
		autoMergeSegments,
		html5ify: ffmpegHtml5ify,
		fixInvalidDuration,
	...
	const onOutFormatLockedClick = () =>
		setOutFormatLocked((v) => (v ? undefined : fileFormat));
	...
	const toggleMute = useCallback(() => {
		setMuted((v) => {
			if (!v && !hideAllNotifications)
				toast.fire({
					icon: "info",
					title: i18n.t("Muted preview (exported file will not be affected)"),
				});
			return !v;
		});
	...
	const effectiveRotation = isRotationSet
		? rotation
		: mainVideoStream &&
		  mainVideoStream.tags &&
		  mainVideoStream.tags.rotate &&
		  parseInt(mainVideoStream.tags.rotate, 10);

	const zoomRel = useCallback(
		(rel) => setZoom((z) => Math.min(Math.max(z + rel, 1), zoomMax)),
		[]
	);
	const canvasPlayerRequired = !!(mainVideoStream && usingDummyVideo);
	const canvasPlayerWanted = !!(
		mainVideoStream &&
		isRotationSet &&
		!hideCanvasPreview
	);
	...
	const comfortZoom = isDurationValid(duration)
		? Math.max(duration / 100, 1)
		: undefined;
	...
	const inverseCutSegments = useMemo(() => {
		...
		return inverted.map((seg) => ({
			...seg,
			segId: `${seg.start}-${seg.end}`,
		}));
	...
	const onLabelSegmentPress = useCallback(
		async (index) => {
	...
	const formatTimecode = useCallback(
		(sec) =>
			formatDuration({
				seconds: sec,
				fps: timecodeShowFrames ? detectedFps : undefined,
			}),
	...
	const setCutStart = useCallback(() => {
		...
		if (
			currentCutSeg.end != null &&
			currentTimeRef.current > currentCutSeg.end
		) {
			addCutSegment();
		} else {
			try {
	...
	const getEdlFilePath = useCallback(
		(fp) => getOutPath(customOutDir, fp, "llc-edl.csv"),
		[customOutDir]
	);
	...
	const [debouncedSaveOperation] = useDebounce(
	...
	useEffect(() => {
		async function save() {
			...
			const {
				cutSegments: saveOperationCutSegments,
				edlFilePath: saveOperationEdlFilePath,
			...
			try {
				...
				if (
					isEqual(
						getCleanCutSegments(saveOperationCutSegments),
						getCleanCutSegments(createInitialCutSegments())
					)
				)
					return;
				...
				if (
					...
					lastSaveOperation.current.edlFilePath === saveOperationEdlFilePath &&
					isEqual(
						getCleanCutSegments(lastSaveOperation.current.cutSegments),
						getCleanCutSegments(saveOperationCutSegments)
					)
				) {
	...
	const increaseRotation = useCallback(() => {
		...
		if (!supportsRotation && !hideAllNotifications)
			toast.fire({
				text: i18n.t(
	...
	const assureOutDirAccess = useCallback(
		async (outFilePath) => {
	...
	const mergeFiles = useCallback(
		async ({ paths, allStreams, segmentsToChapters: segmentsToChapters2 }) => {
			try {
				...
				if (segmentsToChapters2) {
					...
					chapters = await createChaptersFromSegments({
						segmentPaths: paths,
						chapterNames,
					});
				}
				...
				openDirToast({
					icon: "success",
					dirPath: outDir,
					text: i18n.t("Files merged!"),
				});
			} catch (err) {
				errorToast(
					i18n.t(
						"Failed to merge files. Make sure they are all of the exact same codecs"
					)
				);
	...
	const toggleCaptureFormat = useCallback(
		() => setCaptureFormat((f) => (f === "png" ? "jpeg" : "png")),
		[setCaptureFormat]
	);
	...
	const isCopyingStreamId = useCallback(
		(path, streamId) => !!(copyStreamIdsByFile[path] || {})[streamId],
		[copyStreamIdsByFile]
	);

	const copyAnyAudioTrack = mainStreams.some(
		(stream) =>
			isCopyingStreamId(filePath, stream.index) && stream.codec_type === "audio"
	);

	// Streams that are not copy enabled by default
	const extraStreams = useMemo(
		() =>
			mainStreams.filter(
				(stream) => !defaultProcessedCodecTypes.includes(stream.codec_type)
			),
		[mainStreams]
	);

	// Extra streams that the user has not selected for copy
	const nonCopiedExtraStreams = useMemo(
		() =>
			extraStreams.filter(
				(stream) => !isCopyingStreamId(filePath, stream.index)
			),
		[extraStreams, filePath, isCopyingStreamId]
	);

	const exportExtraStreams =
		autoExportExtraStreams && nonCopiedExtraStreams.length > 0;

	const copyFileStreams = useMemo(
		() =>
			Object.entries(copyStreamIdsByFile).map(([path, streamIdsMap]) => ({
				path,
				streamIds: Object.keys(streamIdsMap).filter(
					(index) => streamIdsMap[index]
				),
			})),
		[copyStreamIdsByFile]
	);

	const numStreamsToCopy = copyFileStreams.reduce(
		(acc, { streamIds }) => acc + streamIds.length,
		0
	);
		...
		setCopyStreamIdsForPath(filePath, (old) => {
			...
			mainStreams.forEach((stream) => {
				if (stream.codec_type === "audio")
					newCopyStreamIds[stream.index] = !copyAnyAudioTrack;
			});
	...
	const removeCutSegment = useCallback(
		(index) => {
			if (
				...
				cutSegments[0].start == null &&
				cutSegments[0].end == null
			)
	...
	const [, cancelRenderThumbnails] = useDebounceOld(
		() => {
			async function renderThumbnails() {
				...
				try {
					setThumbnails([]);
					const promise = ffmpegRenderThumbnails({
						filePath,
						from: zoomWindowStartTime,
						duration: zoomedDuration,
						onThumbnail: addThumbnail,
					});
		...
		500,
		[zoomedDuration, filePath, zoomWindowStartTime, thumbnailsEnabled]
	);

	// Cleanup removed thumbnails
	useEffect(() => {
		thumnailsRef.current.forEach((thumbnail) => {
			if (!thumbnails.some((t) => t.url === thumbnail.url))
	...
	const shouldShowKeyframes =
		keyframesEnabled &&
		!!mainVideoStream &&
		calcShouldShowKeyframes(zoomedDuration);
	const shouldShowWaveform = calcShouldShowWaveform(zoomedDuration);

	const { neighbouringFrames, findNearestKeyFrameTime } = useKeyframes({
		keyframesEnabled,
		filePath,
		commandedTime,
		mainVideoStream,
	...
	const createDummyVideo = useCallback(
		async (cod, fp) => {
			const html5ifiedDummyPath = getOutPath(
				cod,
				fp,
				`${html5ifiedPrefix}${html5dummySuffix}.mkv`
			);
			try {
				setCutProgress(0);
				await html5ifyDummy({
					filePath: fp,
					outPath: html5ifiedDummyPath,
					onProgress: setCutProgress,
				});
			} finally {
				setCutProgress();
	...
	const closeFile = useCallback(() => {
		...
		if (
			askBeforeClose &&
			!window.confirm(
				i18n.t("Are you sure you want to close the current file?")
			)
		)
			return false;
	...
	const cleanupFiles = useCallback(async () => {
		...
		try {
			...
			if (deleteTmpFiles && saved.previewFilePath)
				await trash(saved.previewFilePath).catch(console.error);
			if (deleteProjectFile && saved.edlFilePath)
				await trash(saved.edlFilePath).catch(console.error);
			...
			try {
				...
				if (value) {
					if (deleteTmpFiles && saved.previewFilePath)
						await unlink(saved.previewFilePath).catch(console.error);
					if (deleteProjectFile && saved.edlFilePath)
						await unlink(saved.edlFilePath).catch(console.error);
	...
	const outSegments = useMemo(
		() => (invertCutSegments ? inverseCutSegments : apparentCutSegments),
		[invertCutSegments, inverseCutSegments, apparentCutSegments]
	);

	// For invertCutSegments we do not support filtering
	const enabledOutSegmentsRaw = useMemo(
		() =>
			invertCutSegments
				? outSegments
				: outSegments.filter((s) => !disabledSegmentIds[s.segId]),
		[outSegments, invertCutSegments, disabledSegmentIds]
	);
	// If user has selected none to export, it makes no sense, so export all instead
	const enabledOutSegments =
		enabledOutSegmentsRaw.length > 0 ? enabledOutSegmentsRaw : outSegments;

	const onExportSingleSegmentClick = useCallback(
		(activeSeg) =>
			setDisabledSegmentIds(
				Object.fromEntries(
					cutSegments
						.filter((s) => s.segId !== activeSeg.segId)
						.map((s) => [s.segId, true])
				)
			),
		[cutSegments]
	);
	const onExportSegmentEnabledToggle = useCallback(
		(toggleSeg) =>
			setDisabledSegmentIds((existing) => ({
				...existing,
				[toggleSeg.segId]: !existing[toggleSeg.segId],
			})),
		[]
	);
	const onExportSegmentDisableAll = useCallback(
		() =>
			setDisabledSegmentIds(
	...
	const onExportSegmentEnableAll = useCallback(
		() => setDisabledSegmentIds({}),
		[]
	);

	const generateOutSegFileNames = useCallback(
		({ segments = enabledOutSegments, template }) =>
			segments.map(({ start, end, name = "" }, i) => {
				const cutFromStr = formatDuration({
					seconds: start,
					fileNameFriendly: true,
				});
				const cutToStr = formatDuration({
					seconds: end,
					fileNameFriendly: true,
				});
				const segNum = i + 1;
				...
				const generated = generateSegFileName({
					template,
					segSuffix,
					inputFileNameWithoutExt: fileNameWithoutExt,
					ext,
					segNum,
					segLabel: filenamify(name),
					cutFrom: cutFromStr,
					cutTo: cutToStr,
				});
	...
	const isOutSegFileNamesValid = useCallback(
		(fileNames) =>
			fileNames.every((fileName) => {
				...
				const sameAsInputPath =
					pathNormalize(pathJoin(outputDir, fileName)) ===
	...
	const handleCutFailed = useCallback(
		async (err) => {
	...
	const onExportConfirm = useCallback(async () => {
		...
		try
		{
			...
			let outSegFileNames = generateOutSegFileNames({
				segments: enabledOutSegments,
				template: outSegTemplateOrDefault,
			});
			if (
				!isOutSegFileNamesValid(outSegFileNames) ||
				hasDuplicates(outSegFileNames)
			)
			{
				console.error(
				...
				outSegFileNames = generateOutSegFileNames({
					segments: enabledOutSegments,
			...
			if (outFiles.length > 1 && autoMerge)
			{
				...
				const chapterNames =
					segmentsToChapters && !invertCutSegments
						? enabledOutSegments.map((s) => s.name)
						: undefined;
			...
			const extraIphoneMsg = isIphoneHevc(fileFormatData, mainStreams)
				? ` ${i18n.t(
					"There is a known issue with cutting iPhone HEVC videos. The output file may not work in all players."
				)}`
				: "";
			const extraStreamsMsg = exportExtraStreams
				? ` ${i18n.t("Unprocessable streams were exported as separate files.")}`
				: "";

			if (!hideAllNotifications)
			{
				let segmentsFileSize = 0;

				await outFiles.every(async outFile => {
					segmentsFileSize += await statSync(outFile).size;
				});

				const inputFileSize = await statSync(filePath).size;

				outFiles.every(async outFile =>
				{
					con.log(outFile);
					const process = spawn('C:\\Program Files\\Windows Media Player\\wmplayer.exe', [outFile]);
					...
					{
						con.error(data.toString());
					});

					process.on('exit', (code) =>
					{
						con.log(`Child exited with code ${code}`);
					});
				});

				if (await openDirToast({
					dirPath: outputDir,
					confirmButtonText: i18n.t('Replace original file'),
					text: `${i18n.t(
					...
					)}${extraIphoneMsg}${extraStreamsMsg} ${i18n.t("Output segment file size compared to original input video:")} ${parseFloat((segmentsFileSize / inputFileSize) * 100).toFixed(1)}%`,
					timer: 210000,
				}))
				{
					if (window.confirm(
						`Delete ${filePath} and rename ${outFiles[0]}?`)
					)
					{
						unlinkSync(filePath);
						renameSync(outFiles[0], filePath);
					}
				}
			}
		} catch (err) {
	...
	const capture = useCallback(async () => {
		...
		try {
			...
			openDirToast({
				dirPath: outputDir,
				text: `${i18n.t("Screenshot captured to:")} ${outPath}`,
			});
		} catch (err) {
			console.error(err);
	...
	const changePlaybackRate = useCallback(
		(dir) => {
			if (canvasPlayerEnabled) {
				toast.fire({
					title: i18n.t("Unable to change playback rate right now"),
					timer: 1000,
				});
				return;
			}
			...
			if (!playing) {
				...
				const newRate = clamp(
					Math.round((video.playbackRate + dir * 0.15) * 100) / 100,
					0.1,
					16
				);
				toast.fire({
					title: `${i18n.t("Playback rate:")} ${Math.round(newRate * 100)}%`,
					timer: 1000,
				});
	...
	const splitCurrentSegment = useCallback(() => {
		...
		const getNewName = (oldName, suffix) =>
			oldName && `${segmentAtCursor2.name} ${suffix}`;

		const firstPart = createSegment({
			name: getNewName(segmentAtCursor2.name, "1"),
			start: segmentAtCursor2.start,
			end: currentTimeRef.current,
		});
		const secondPart = createSegment({
			name: getNewName(segmentAtCursor2.name, "2"),
			start: currentTimeRef.current,
			end: segmentAtCursor2.end,
		});
	...
	const load = useCallback(
		async ({
			filePath: fp,
			customOutDir: cod,
			html5FriendlyPathRequested,
			dummyVideoPathRequested,
		}) => {
			console.log("Load", {
				fp,
				cod,
			...
			async function checkAndSetExistingHtml5FriendlyFile() {
				...
				for (const entry of dirEntries) {
					...
					if (prefixMatch) {
						const speedMatch = speeds.find((s) =>
							new RegExp(`${s}\\..*$`).test(entry.replace(prefix, ""))
						);
						if (!speedMatch || speedMatch !== html5dummySuffix) {
							// skip dummy, as it's not very useful
			...
			try {
				...
				const videoStream = streams.find(
					(stream) =>
						stream.codec_type === "video" && !isStreamThumbnail(stream)
				);
				...
				const shouldCopyStreamByDefault = (stream) => {
					if (!defaultProcessedCodecTypes.includes(stream.codec_type))
						return false;
				...
				if (!isAudioSupported(streams)) {
					toast.fire({
						icon: "info",
						text: i18n.t(
							"The audio track is not supported. You can convert to a supported format from the menu"
						),
					});
				}
	...
	const jumpSeg = useCallback(
		(val) =>
			setCurrentSegIndex((old) =>
				Math.max(Math.min(old + val, cutSegments.length - 1), 0)
			),
		[cutSegments.length]
	);
	...
	useEffect(() => {
		...
		const seekBackwardsPercent = () => {
			seekRelPercent(-0.01);
			return false;
		};
		const seekForwardsPercent = () => {
			seekRelPercent(0.01);
			return false;
		};
	...
	const extractSingleStream = useCallback(
		async (index) => {
			...
			try {
				...
				openDirToast({
					dirPath: outputDir,
					text: i18n.t("Track has been extracted"),
				});
			} catch (err) {
	...
	const extractAllStreams = useCallback(async () => {
		...
		try {
			...
			openDirToast({
				dirPath: outputDir,
				text: i18n.t("All streams have been extracted as separate files"),
			});
		} catch (err) {
	...
	const addStreamSourceFile = useCallback(
		async (path) => {
			...
			setExternalStreamFiles((old) => ({
				...old,
				[path]: { streams, formatData },
			}));
			setCopyStreamIdsForPath(path, () =>
				fromPairs(streams.map(({ index }) => [index, true]))
			);
		},
		[externalStreamFiles]
	);
	...
	const userOpenFiles = useCallback(
		async (filePathsRaw) => {
			...
			const filePaths = filePathsRaw.map((path) =>
				pathIsAbsolute(path) ? path : pathResolve(path)
			);
			...
			const openFileResponse = enableAskForFileOpenAction
				? await askForFileOpenAction()
				: "open";
	...
	const onDrop = useCallback(
		async (ev) => {
			...
			if (
				filePaths.length === 1 &&
				filePaths[0].toLowerCase().endsWith(".csv")
			) {
	...
	const html5ify = useCallback(
		async ({
			...
			if (ha) {
				...
				else if (speed === "slow-audio" || speed === "fastest-audio")
					audio = "lq";
	...
	const html5ifyCurrentFile = useCallback(async () => {
		...
		try {
			...
			const speed = await askForHtml5ifySpeed([
				"fastest",
				"fastest-audio",
				"fast-audio",
				"fast",
	...
	const onVideoError = useCallback(async () => {
		...
		function showToast() {
			...
			if (!hideAllNotifications)
				toast.fire({
					icon: "info",
					text: "This file is not natively supported. Creating a preview file...",
				});
		}
	...
	useEffect(() => {
		...
		async function setStartOffset() {
			const newStartTimeOffset = await promptTimeOffset(
				startTimeOffset !== undefined
					? formatDuration({ seconds: startTimeOffset })
					: undefined
			);
		...
		async function importEdlFile(e, type) {
			...
			else if (type === "mplayer")
				filters = [{ name: i18n.t("MPlayer EDL"), extensions: ["*"] }];
		...
		const fileOpened = (event, filePaths) => {
			userOpenFiles(filePaths);
		};
		const showStreamsSelector = () => setStreamsSelectorShown(true);
		const openSendReportDialog2 = () => {
	...
	const commonFormatsMap = useMemo(
		() =>
			fromPairs(
				commonFormats
					.map((format) => [format, allOutFormats[format]])
					.filter(([f]) => f !== detectedFileFormat)
			),
		[detectedFileFormat]
	);

	const otherFormatsMap = useMemo(
		() =>
			fromPairs(
				Object.entries(allOutFormats).filter(
					([f]) => ![...commonFormats, detectedFileFormat].includes(f)
				)
			),
		[detectedFileFormat]
	);
			...
			>
				...
				{detectedFileFormat && (
					<option key={detectedFileFormat} value={detectedFileFormat}>
						{detectedFileFormat} - {allOutFormats[detectedFileFormat]}{" "}
						{i18n.t("(detected)")}
					</option>
				)}

				<option key="disabled2" value="" disabled>
					--- {i18n.t("Common formats:")} ---
				</option>
				{renderFormatOptions(commonFormatsMap)}

				<option key="disabled3" value="" disabled>
					--- {i18n.t("All formats:")} ---
				</option>
	...
	const renderCaptureFormatButton = useCallback(
		(props) => (
			<Button
	...
	const AutoExportToggler = useCallback(
		() => (
			<SegmentedControl
				options={[
					{ label: i18n.t("Extract"), value: "extract" },
	...
	const bottomBarHeight =
		96 +
		((hasAudio && waveformEnabled) || (hasVideo && thumbnailsEnabled)
			? timelineHeight
			: 0);

	const thumbnailsSorted = useMemo(
		() => sortBy(thumbnails, (thumbnail) => thumbnail.time),
		[thumbnails]
	);
		...
		<div>
					...
					>
						<motion.div
							style={{
								...
								boxShadow: `${primaryColor} 0px 0px 20px 25px`,
								borderRadius: 20,
								paddingBottom: 15,
								color: "white",
								textAlign: "center",
						...
						>
							...
							<div style={{ marginTop: 10, width: 150 }}>{working}...</div>
			...
			<div
				...
				style={{
					...
					right: sideBarWidth,
					bottom: bottomBarHeight,
					visibility: !isFileOpened ? "hidden" : undefined,
				}}
				...
				>
					...
					{!canvasPlayerRequired && (
						<FaWindowClose
							...
							onClick={() => setHideCanvasPreview(true)}
						/>
					)}
				</div>
			)}
			...
			<ExportConfirm
				...
				togglePreserveMovData={togglePreserveMovData}
				movFastStart={movFastStart}
				toggleMovFastStart={toggleMovFastStart}
				avoidNegativeTs={avoidNegativeTs}
				setAvoidNegativeTs={setAvoidNegativeTs}
				changeOutDir={changeOutDir}
				outputDir={outputDir}
				numStreamsTotal={numStreamsTotal}
				numStreamsToCopy={numStreamsToCopy}
				...
				exportConfirmEnabled={exportConfirmEnabled}
				toggleExportConfirmEnabled={toggleExportConfirmEnabled}
				segmentsToChapters={segmentsToChapters}
				toggleSegmentsToChapters={toggleSegmentsToChapters}
				outFormat={fileFormat}
				preserveMetadataOnMerge={preserveMetadataOnMerge}
				togglePreserveMetadataOnMerge={togglePreserveMetadataOnMerge}
				setOutSegTemplate={setOutSegTemplate}
				outSegTemplate={outSegTemplateOrDefault}
				generateOutSegFileNames={generateOutSegFileNames}
				currentSegIndexSafe={currentSegIndexSafe}
				isOutSegFileNamesValid={isOutSegFileNamesValid}
				autoDeleteMergedSegments={autoDeleteMergedSegments}
				setAutoDeleteMergedSegments={setAutoDeleteMergedSegments}
			/>
