...
export const openDirToast = async ({ dirPath, confirmButtonText = i18n.t('Show'), timer = 13000, ...props }) => {
  const { value } = await toast.fire({ icon: 'success', ...props, timer, showConfirmButton: true, confirmButtonText, showCancelButton: true, cancelButtonText: i18n.t('Close') });
	if (value && confirmButtonText === i18n.t('Show'))
	{
		open(dirPath);
	}
	return value;
